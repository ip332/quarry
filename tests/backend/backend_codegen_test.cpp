...
TEST(BackendCodegenTest, GeneratedScalarEncoderEmitsExactBytes) {
    ...
    const std::string translation_source =
        ...
        "      byte(0x04), byte(0x3F), byte(0xF8), byte(0x00),\n"
        "      byte(0x00), byte(0x00), byte(0x00), byte(0x00),\n"
        "      byte(0x00)};\n"
...
TEST(BackendCodegenTest, GeneratedScalarEncoderDistinguishesAbsentAndPresentDefault) {
    ...
    const std::string translation_source =
        ...
        "  if (!empty_encoded.has_value() || empty_encoded->size() != 30U) {\n"
        "    return 1;\n"
        "  }\n"
        "  ::ExampleBuilder builder;\n"
        "  if (!builder.set_active(false)) {\n"
...
TEST(BackendCodegenTest, GeneratedScalarDecoderReadsManualBytes) {
    ...
    const std::string translation_source =
        ...
        "      byte(0x00), byte(0x00), byte(0x00), byte(0x0E),\n"
        "      byte(0x00), byte(0x00), byte(0x00), byte(0x1E),\n"
        "      byte(0x07), byte(0x00), byte(0x01), byte(0x02),\n"
        "      byte(0x03), byte(0x04), byte(0x3F), byte(0xF8),\n"
        "      byte(0x00), byte(0x00), byte(0x00), byte(0x00),\n"
        "      byte(0x00), byte(0x00)};\n"
...
TEST(BackendCodegenTest, GeneratedScalarDecoderReportsFieldPayloadByteOffset) {
    ...
    const std::string translation_source =
        ...
        "  if (decoded.error != ::quarry::runtime::DecodeError::invalid_payload_length) {\n"
        "    return 2;\n"
        "  }\n"
        "  if (!decoded.path.empty()) {\n"
        "    return 3;\n"
        "  }\n"
        "  if (!decoded.byte_offset.has_value() || *decoded.byte_offset != 8U) {\n"
        "    return 4;\n"
        "  }\n"
        "  return 0;\n"
        "}\n";
...
TEST(BackendCodegenTest, GeneratedDiagnosticDecodeReportsStructuralAndSchemaErrors) {
    ...
    const std::string translation_source =
        ...
        "    return static_cast<std::byte>(static_cast<unsigned char>(value));\n"
        "  };\n"
        "  const auto put_u32 = [&](std::vector<std::byte>& value, std::size_t offset, unsigned int number) {\n"
        "    value[offset + 0U] = byte(number >> 24U);\n"
        "    value[offset + 1U] = byte(number >> 16U);\n"
        "    value[offset + 2U] = byte(number >> 8U);\n"
        "    value[offset + 3U] = byte(number);\n"
        "  };\n"
        "  const auto make_variable = [&](std::size_t descriptor, unsigned int presence_bit, std::vector<std::byte> payload) {\n"
        "    std::vector<std::byte> value(107U, byte(0));\n"
        "    value[0] = byte(0x02); value[3] = byte(0x10);\n"
        "    put_u32(value, 4U, 2U); put_u32(value, 8U, 91U); put_u32(value, 12U, 107U);\n"
        "    value[16U + presence_bit / 8U] = byte(1U << (presence_bit % 8U));\n"
        "    put_u32(value, descriptor, 107U); put_u32(value, descriptor + 4U, payload.size());\n"
        "    value.insert(value.end(), payload.begin(), payload.end());\n"
        "    put_u32(value, 12U, value.size());\n"
        "    return value;\n"
        "  };\n"
        "  const auto empty_record = [] { return std::vector<std::byte>(107U, std::byte{0}); };\n"
        ...
        "    return 1;\n"
        "  }\n"
        "  std::vector<std::byte> wrong_record_id = empty_record();\n"
        "  wrong_record_id[0] = byte(0x02); wrong_record_id[3] = byte(0x10);\n"
        "  wrong_record_id[7] = byte(0x03); put_u32(wrong_record_id, 8U, 91U);\n"
        "  put_u32(wrong_record_id, 12U, 107U);\n"
        "  if (decode_Example_result(wrong_record_id).error != DecodeError::unexpected_record_id) {\n"
        "    return 2;\n"
        "  }\n"
        "  std::vector<std::byte> invalid_bool = empty_record();\n"
        "  invalid_bool[0] = byte(0x02); invalid_bool[3] = byte(0x10);\n"
        "  put_u32(invalid_bool, 4U, 2U); put_u32(invalid_bool, 8U, 91U); put_u32(invalid_bool, 12U, 107U);\n"
        "  invalid_bool[17] = byte(0x02); invalid_bool[90] = byte(0x02);\n"
        "  if (decode_Example_result(invalid_bool).error != DecodeError::invalid_bool) {\n"
        "    return 3;\n"
        "  }\n"
        "  const auto invalid_utf8 = make_variable(18U, 0U, {byte(0xC0), byte(0x80)});\n"
        ...
        "    return 5;\n"
        "  }\n"
        "  const auto over_bound_string = make_variable(18U, 0U, std::vector<std::byte>(17U, byte(0x61)));\n"
        "  if (decode_Example_result(over_bound_string).error != DecodeError::bounds_exceeded) {\n"
        "    return 6;\n"
        "  }\n"
        "  const auto count_over_bound = make_variable(34U, 2U, {byte(0x05)});\n"
        "  if (decode_Example_result(count_over_bound).error != DecodeError::bounds_exceeded) {\n"
        "    return 7;\n"
        "  }\n"
        "  const auto unknown_enum = make_variable(50U, 4U, {byte(0x01), byte(0x07)});\n"
...
TEST(BackendCodegenTest, NestedRecordFieldUsesCompleteEmbeddedRecord) {
    ...
    compile_generated_header(
        ...
        "  std::vector<byte> expected_empty(38U, byte{0});\n"
        "  expected_empty[0] = byte{0x02}; expected_empty[3] = byte{0x10};\n"
        "  expected_empty[7] = byte{0x01}; expected_empty[11] = byte{0x16};\n"
        "  expected_empty[15] = byte{0x26}; expected_empty[16] = byte{0x01};\n"
        "  expected_empty[17] = byte{0x02}; expected_empty[20] = byte{0x10};\n"
        "  expected_empty[24] = byte{0x02}; expected_empty[28] = byte{0x05};\n"
        "  expected_empty[32] = byte{0x15};\n"
        ...
        "  std::vector<byte> expected_populated(38U, byte{0});\n"
        "  expected_populated[0] = byte{0x02}; expected_populated[3] = byte{0x10};\n"
        "  expected_populated[7] = byte{0x01}; expected_populated[11] = byte{0x16};\n"
        "  expected_populated[15] = byte{0x26}; expected_populated[16] = byte{0x01};\n"
        "  expected_populated[17] = byte{0x02}; expected_populated[20] = byte{0x10};\n"
        "  expected_populated[24] = byte{0x02}; expected_populated[28] = byte{0x05};\n"
        "  expected_populated[32] = byte{0x15}; expected_populated[33] = byte{0x01};\n"
        "  expected_populated[34] = byte{0x01}; expected_populated[35] = byte{0x02};\n"
        "  expected_populated[36] = byte{0x03}; expected_populated[37] = byte{0x04};\n"
        ...
        "  }\n"
        "\n"
        "  std::vector<byte> wrong_nested_id = expected_populated;\n"
        "  wrong_nested_id[24] = byte{0x03};\n"
        "  if (decode_A(wrong_nested_id).has_value()) { return 8; }\n"
        "\n"
        "  std::vector<byte> truncated_nested_header = expected_populated;\n"
        "  truncated_nested_header[32] = byte{0x10};\n"
        ...
        "\n"
        "  std::vector<byte> unknown_nested_field(38U, byte{0});\n"
        "  unknown_nested_field[0] = byte{0x02}; unknown_nested_field[3] = byte{0x10};\n"
        "  unknown_nested_field[7] = byte{0x01}; unknown_nested_field[11] = byte{0x16};\n"
        "  unknown_nested_field[15] = byte{0x26};\n"
...
TEST(BackendCodegenTest, CrossNamespaceNestedRecordCodecCompilesAndRoundTrips) {
    ...
    compile_generated_header(
        ...
        "  std::vector<byte> expected(33U, byte{0});\n"
        "  expected[0] = byte{0x02}; expected[3] = byte{0x10}; expected[7] = byte{0x02};\n"
        "  expected[11] = byte{0x11}; expected[15] = byte{0x21}; expected[16] = byte{0x01};\n"
        "  expected[17] = byte{0x02}; expected[20] = byte{0x10}; expected[24] = byte{0x01};\n"
        "  expected[32] = byte{0x10};\n"
...
TEST(BackendCodegenTest, GeneratedDiagnosticCodecsPropagateNestedAndRecordArrayErrors) {
    ...
    compile_generated_header(
        ...
        "  if (wrong_nested_result.value.has_value()) { return 7; }\n"
        "\n"
        ...
        "  if (wrong_array_result.value.has_value()) { return 8; }\n"
        "  return 0;\n"
        "}\n");
}
...
TEST(BackendCodegenTest, GeneratedRecordArrayCodecPreservesPresenceAndElements) {
    ...
    const std::string translation_source =
        ...
        "  const auto absent = encode(absent_builder.build());\n"
        "  if (!absent.has_value()) { return 1; }\n"
        ...
        "  if (!empty_array.has_value() || empty_array->size() != 108U ||\n"
        "      (*empty_array)[16] != byte(0x20) || (*empty_array)[107] != byte(0x00)) { return 3; }\n"
        ...
        "  }\n"
        "  const auto encoded = encode(builder.build());\n"
        "  if (!encoded.has_value() || encoded->size() != 150U || (*encoded)[16] != byte(0x20) ||\n"
        "      (*encoded)[107] != byte(0x02) || (*encoded)[108] != byte(0x02)) { return 7; }\n"
        "  const auto decoded = decode_Example(*encoded);\n"
...
TEST(BackendCodegenTest, GeneratedStringAndBytesEncoderEmitsExactBytes) {
    ...
    const std::string translation_source =
        ...
        "    return 3;\n"
        "  }\n"
        "  if (encoded->size() != 115U || (*encoded)[0] != byte(0x02) ||\n"
        "      (*encoded)[3] != byte(0x10) || (*encoded)[7] != byte(0x02) ||\n"
        "      (*encoded)[16] != byte(0x03) || (*encoded)[17] != byte(0x00) ||\n"
        "      (*encoded)[107] != byte(0x68) || (*encoded)[108] != byte(0x69) ||\n"
        "      (*encoded)[109] != byte(0x00) || (*encoded)[110] != byte(0xC2) ||\n"
        "      (*encoded)[111] != byte(0xA2) || (*encoded)[112] != byte(0x00) ||\n"
        "      (*encoded)[113] != byte(0xFF) || (*encoded)[114] != byte(0x80)) {\n"
        "    return 4;\n"
        "  }\n"
...
TEST(BackendCodegenTest, GeneratedStringAndBytesEncoderDistinguishesEmptyPresence) {
    ...
    const std::string translation_source =
        ...
        "  ::ExampleBuilder empty_builder;\n"
        "  const auto empty_encoded = encode(empty_builder.build());\n"
        "  if (!empty_encoded.has_value() || empty_encoded->size() != 107U) {\n"
        "    return 1;\n"
        "  }\n"
        "  ::ExampleBuilder builder;\n"
        ...
        "    return 3;\n"
        "  }\n"
        "  if (encoded->size() != 107U || (*encoded)[0] != byte(0x02) ||\n"
        "      (*encoded)[3] != byte(0x10) || (*encoded)[7] != byte(0x02) ||\n"
        "      (*encoded)[16] != byte(0x03) || (*encoded)[17] != byte(0x00) ||\n"
        "      (*encoded)[21] != byte(0x6B) || (*encoded)[25] != byte(0x00) ||\n"
        "      (*encoded)[29] != byte(0x6B) || (*encoded)[33] != byte(0x00)) {\n"
        "    return 4;\n"
        "  }\n"
        "  const auto decoded = decode_Example(*encoded);\n"
...
TEST(BackendCodegenTest, GeneratedStringAndBytesDecoderReadsManualBytes) {
    ...
    const std::string translation_source =
        ...
        "  };\n"
        "  ::ExampleBuilder builder;\n"
        "  if (!builder.set_name(std::string(\"hi\\0\\xC2\\xA2\", 5U)) ||\n"
        "      !builder.set_payload(std::vector<std::byte>{byte(0x00), byte(0xFF), byte(0x80)})) {\n"
        "    return 1;\n"
        "  }\n"
        "  const auto input = encode(builder.build());\n"
...
TEST(BackendCodegenTest, GeneratedArrayEncoderDistinguishesEmptyPresence) {
    ...
    const std::string translation_source =
        ...
        "  if (!absent_encoded.has_value() || absent_encoded->size() != 107U) {\n"
        "    return 1;\n"
        "  }\n"
        "  ::ExampleBuilder builder;\n"
...
TEST(BackendCodegenTest, GeneratedArrayEncoderEmitsExactBytesForFixedWidthElements) {
    ...
    const std::string translation_source =
        ...
        "  if (encoded->empty() || (*encoded)[0] != byte(0x02)) {\n"
        "    return 3;\n"
        "  }\n"
        "  const auto decoded = decode_Example(*encoded);\n"
...
TEST(BackendCodegenTest, GeneratedStringArrayCodecPreservesPresenceAndElements) {
    ...
    const std::string translation_source =
        ...
        "  if (!empty_encoded.has_value()) { return 2; }\n"
        "  ::ExampleBuilder builder;\n"
        ...
        "  if (!encoded.has_value() || encoded->empty() || (*encoded)[0] != byte(0x02)) { return 4; }\n"
        "  const auto decoded = decode_Example(*encoded);\n"
...
TEST(BackendCodegenTest, GeneratedBytesArrayCodecPreservesArbitraryElements) {
    ...
    const std::string translation_source =
        ...
        "      byte(0x01), byte(0xAA)};\n"
        "  if (!encoded.has_value() || encoded->empty() || (*encoded)[0] != byte(0x02)) { return 2; }\n"
        "  const auto decoded = decode_Example(*encoded);\n"
...
TEST(BackendCodegenTest, GeneratedArrayCodecHandlesMixedFixedWidthArrays) {
    ...
    const std::string translation_source =
        ...
        "  if (encoded->empty() || (*encoded)[0] != byte(0x02)) { return 13; }\n"
        "  const auto decoded = decode_Example(*encoded);\n"
...
TEST(BackendCodegenTest, GeneratedArrayDecoderReportsByteOffsetForCountVaruintFailure) {
    ...
    const std::string translation_source =
        ...
        "  if (decoded.value.has_value()) { return 1; }\n"
        "  if (decoded.value.has_value()) { return 2; }\n"
        "  return 0;\n"
        "}\n";
...
TEST(BackendCodegenTest, GeneratedEnumEncoderEmitsExactBytesAndRejectsUnknownValue) {
    ...
    const std::string translation_source =
        ...
        "    return 2;\n"
        "  }\n"
        "  if (encoded->size() != 18U || (*encoded)[0] != byte(0x02) ||\n"
        "      (*encoded)[7] != byte(0x01) || (*encoded)[16] != byte(0x01) ||\n"
        "      (*encoded)[17] != byte(0x02)) {\n"
        "    return 3;\n"
        "  }\n"
...
TEST(BackendCodegenTest, GeneratedEnumDecoderReadsKnownValueAndRejectsUnknownValue) {
    ...
    const std::string translation_source =
        ...
        "    return static_cast<std::byte>(static_cast<unsigned char>(value));\n"
        "  };\n"
        "  ::PaintBuilder known_builder;\n"
        "  if (!known_builder.set_color(::Color::Green)) { return 1; }\n"
        "  const auto known = encode(known_builder.build());\n"
