...
// produces a package that a real Python interpreter can actually import
// and use, matching the project's "verify end-to-end, don't just trust
// generated text" discipline already applied to the C/C++ interop tests.
// Covers: generating through the real quarry-schema-compiler binary and
// importing the result with a real `python3` subprocess; a zero-field
// record's real (not stubbed) encode/decode round trip; a scalar record's
// real encode/decode round trip with representative values; an absent
// scalar field decoding as None; an out-of-range scalar value raising
// EncodeError at encode time; and malformed/truncated/trailing-byte input
// raising DecodeError at decode time.
