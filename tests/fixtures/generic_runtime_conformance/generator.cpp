...
SchemaIR fixed_fixture_schema() {
    ...
    auto* item = add_record(2U, 2U, "FixedItem");
    auto* value = item->add_fields(); value->set_name("value"); value->set_field_index(0U);
    value->mutable_type()->set_primitive(quarry::schema_ir::PRIMITIVE_TYPE_U32);
    auto* parent = add_record(1U, 1U, "FixedParent");
    auto* before = parent->add_fields(); before->set_name("before"); before->set_field_index(0U);
    before->mutable_type()->set_primitive(quarry::schema_ir::PRIMITIVE_TYPE_U8);
    auto* items = parent->add_fields(); items->set_name("items"); items->set_field_index(1U);
    items->mutable_type()->mutable_array()->set_max_elements(4U);
    items->mutable_type()->mutable_array()->mutable_element_type()->mutable_record()->set_target_record_ir_id(2U);
    auto* after = parent->add_fields(); after->set_name("after"); after->set_field_index(2U);
    after->mutable_type()->set_primitive(quarry::schema_ir::PRIMITIVE_TYPE_U8);
    return schema;
}
...
std::vector<std::uint8_t> generate_fixed(std::vector<std::uint8_t>& qbs) {
    ...
    if (!diagnostics.empty()) throw std::runtime_error("fixed layout failed");
    auto model = QbsModelBuilder{}.build(source, layout, {.mode = BuildMode::Reflective}, diagnostics);
    if (!model) throw std::runtime_error("fixed QBS model failed");
    auto image = serialize_qbs(*model, diagnostics);
    if (!image) throw std::runtime_error("fixed QBS serialization failed");
    qbs = image->bytes;
