...
using quarry::tools::protobuf::DescriptorBounds;
...
using quarry::tools::protobuf::OptionsFormat;
...
[[nodiscard]] DescriptorModel representative_model() {
    ...
    sample.fields.push_back(field("child", FieldType::Message, "telemetry.Child"));
...
TEST(ProtobufTranslationTest, EmitsBoundedReachableBundleAndManifest) {
    ...
    EXPECT_EQ(read_text(sample), "namespace: telemetry.sample\n"
                                 "record: Sample\n"
                                 "version: 1\n"
                                 "type: data\n"
                                 "imports:\n"
...
TEST(ProtobufTranslationTest, EmitsOwnedEnumsAndMigrationMetadata) {
    ...
    model.messages[1].fields.push_back(field("mode", FieldType::Enum, "telemetry.Sample.Mode"));
...
TEST(ProtobufTranslationTest, AdaptsNanopbBoundsAndRecordsProvenance) {
    const std::filesystem::path root = temporary_directory("nanopb");
    write_text(root / "common.options", "telemetry.Sample.name max_size:64\n"
                                        "telemetry.Sample.payload max_size:128\n"
                                        "telemetry.Sample.values max_count:4\n"
                                        "telemetry.Sample.children max_count:2\n"
                                        "telemetry.Child.label max_size:32\n");
    const auto result = quarry::tools::protobuf::translate_descriptor_model(
        representative_model(), "telemetry.Sample",
        std::vector<std::string>{(root / "common.options").string()}, OptionsFormat::Nanopb,
        (root / "generated").string());
    ASSERT_TRUE(result.succeeded())
        << (result.diagnostics.empty() ? "" : result.diagnostics.front());
    const std::string sample = read_text(root / "generated/telemetry/sample/sample.brd");
    EXPECT_NE(sample.find("max_bytes: 64"), std::string::npos);
    EXPECT_NE(sample.find("max_elements: 4"), std::string::npos);
    const std::string manifest = read_text(root / "generated/manifest.json");
    EXPECT_NE(manifest.find("\"source_type\": \"nanopb_options\""), std::string::npos);
    EXPECT_NE(manifest.find("\"original_option\": \"max_size:64\""), std::string::npos);
    EXPECT_NE(manifest.find("\"source_line\": 1"), std::string::npos);
}

TEST(ProtobufTranslationTest, NanopbFilesOverrideOnlyExternalValues) {
    const std::filesystem::path root = temporary_directory("nanopb-precedence");
    DescriptorModel model = representative_model();
    DescriptorBounds field_option;
    field_option.max_bytes = 16;
    model.messages[1].fields[1].custom_bounds = field_option;
    write_text(root / "common.options", "telemetry.Sample.name max_size:64\n");
    write_text(root / "board.options", "telemetry.Sample.name max_size:32\n"
                                       "telemetry.Sample.payload max_size:128\n"
                                       "telemetry.Sample.values max_count:4\n"
                                       "telemetry.Sample.children max_count:2\n"
                                       "telemetry.Child.label max_size:32\n");
    const auto result = quarry::tools::protobuf::translate_descriptor_model(
        model, "telemetry.Sample",
        std::vector<std::string>{(root / "common.options").string(),
                                 (root / "board.options").string()},
        OptionsFormat::Nanopb, (root / "generated").string());
    ASSERT_TRUE(result.succeeded())
        << (result.diagnostics.empty() ? "" : result.diagnostics.front());
    const std::string sample = read_text(root / "generated/telemetry/sample/sample.brd");
    EXPECT_NE(sample.find("  name:\n    type: string\n    max_bytes: 16"), std::string::npos);
    const std::string manifest = read_text(root / "generated/manifest.json");
    EXPECT_NE(manifest.find("\"source_type\": \"field_option\""), std::string::npos);
}

TEST(ProtobufTranslationTest, RejectsUnsupportedNanopbOptionsAndWildcards) {
    const std::filesystem::path root = temporary_directory("nanopb-rejection");
    write_text(root / "invalid.options", "telemetry.Sample.* max_count:4\n"
                                         "telemetry.Sample.name max_length:32\n"
                                         "telemetry.Sample.payload callback:foo\n"
                                         "telemetry.Sample.values max_count:4 max_count:8\n"
                                         "telemetry.Sample.children malformed\n");
    const auto result = quarry::tools::protobuf::translate_descriptor_model(
        representative_model(), "telemetry.Sample",
        std::vector<std::string>{(root / "invalid.options").string()}, OptionsFormat::Nanopb,
        (root / "generated").string());
    ASSERT_FALSE(result.succeeded());
    std::ostringstream diagnostics;
    for (const auto& diagnostic : result.diagnostics)
        diagnostics << diagnostic << "\n";
    EXPECT_NE(diagnostics.str().find("wildcards are unsupported"), std::string::npos);
    EXPECT_NE(diagnostics.str().find("max_length"), std::string::npos);
    EXPECT_NE(diagnostics.str().find("callback"), std::string::npos);
    EXPECT_NE(diagnostics.str().find("duplicate max_count"), std::string::npos);
    EXPECT_NE(diagnostics.str().find("malformed option"), std::string::npos);
...
TEST(ProtobufTranslationTest, RejectsNegativeAndAliasedEnums) {
    ...
    invalid.values = {DescriptorEnumValue{"NEGATIVE", -1}, DescriptorEnumValue{"ALIAS", 1},
                      DescriptorEnumValue{"OTHER_ALIAS", 1}};
    model.enums.push_back(invalid);
    model.messages[1].fields.push_back(field("status", FieldType::Enum, "telemetry.InvalidStatus"));
    const std::filesystem::path bounds = root / "bounds.yaml";
    write_text(bounds, "bounds:\n"
                       "  telemetry.Sample.name:\n"
                       "    max_bytes: 64\n"
...
TEST(ProtobufTranslationTest, TranslationIsDeterministicAcrossBoundsOrdering) {
    ...
    const std::string first_bounds = "bounds:\n"
                                     "  telemetry.Sample.name:\n"
                                     "    max_bytes: 64\n"
                                     "  telemetry.Sample.payload:\n"
                                     "    max_bytes: 256\n"
                                     ...
                                     "  telemetry.Child.label:\n"
                                     "    max_bytes: 32\n";
    const std::string second_bounds = "bounds:\n"
                                      "  telemetry.Child.label:\n"
                                      "    max_bytes: 32\n"
                                      "  telemetry.Sample.children:\n"
                                      "    max_elements: 2\n"
    ...
    ASSERT_TRUE(quarry::tools::protobuf::translate_descriptor_model(
                    representative_model(), "telemetry.Sample",
                    (root / "first" / "bounds.yaml").string(), (root / "first").string())
                    .succeeded());
    ASSERT_TRUE(quarry::tools::protobuf::translate_descriptor_model(
                    representative_model(), "telemetry.Sample",
                    (root / "second" / "bounds.yaml").string(), (root / "second").string())
                    .succeeded());
...
TEST(ProtobufTranslationTest, RejectsRecursiveMessagesBeforeWriting) {
    ...
    DescriptorModel model = representative_model();
    model.messages[1].fields.push_back(field("self", FieldType::Message, "telemetry.Sample"));
    const std::filesystem::path bounds = root / "bounds.yaml";
    write_text(bounds, "bounds:\n"
                       "  telemetry.Sample.name:\n"
                       "    max_bytes: 64\n"
    ...
    const auto result = quarry::tools::protobuf::translate_descriptor_model(
        model, "telemetry.Sample", bounds.string(), (root / "generated").string());
    ASSERT_FALSE(result.succeeded());
    EXPECT_NE(result.diagnostics.front().find("recursive protobuf message graph"),
              std::string::npos);
    EXPECT_FALSE(std::filesystem::exists(root / "generated"));
}
...
TEST(ProtobufTranslationTest, ProtocTranslationAndCompilerFollowThrough) {
    ...
    write_text(root / "schema.proto", "syntax = \"proto3\";\n"
                                      "package telemetry;\n"
    ...
    write_text(root / "nanopb.options", "telemetry.Sample.name max_size:64\n"
                                        "telemetry.Sample.payload max_size:128\n"
                                        "telemetry.Sample.values max_count:4\n"
                                        "telemetry.Sample.children max_count:2\n"
                                        "telemetry.Sample.statuses max_count:3\n"
                                        "telemetry.Child.label max_size:32\n");
    const std::filesystem::path nanopb_generated = root / "nanopb-generated";
    ASSERT_EQ(std::system((translator + " --descriptor-set " + shell_quote(descriptor.string()) +
                           " --root telemetry.Sample --options " +
                           shell_quote((root / "nanopb.options").string()) +
                           " --options-format nanopb --output-dir " +
                           shell_quote(nanopb_generated.string()))
                              .c_str()),
              0);
    EXPECT_TRUE(std::filesystem::exists(nanopb_generated / "manifest.json"));
    EXPECT_NE(
        read_text(nanopb_generated / "manifest.json").find("\"source_type\": \"nanopb_options\""),
        std::string::npos);
}
