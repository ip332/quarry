...
private:
    ...
    [[nodiscard]] SchemaIrBuildState state() {
        return SchemaIrBuildState{.source_manager = &source_manager_,
                                  .diagnostics = &diagnostics_,
                                  .failed = &layout_failed_,
                                  .next_ir_id = &next_ir_id_,
                                  .ir_ids_by_fqn = &ir_ids_by_fqn_};
    }
    ...
    bool populate_record_fields(::breadcrumbs::schema_ir::RecordIR& record,
        ...
        for (std::size_t field_index = 0; field_index < declaration.fields.size(); ++field_index) {
            ...
            field_ir->mutable_type()->CopyFrom(
                lower_semantic_type_with_state(semantic_field.type, record_fqn,
                                               field.name.text, field.source_range, state()));
            if (layout_failed_) {
                return false;
            }
