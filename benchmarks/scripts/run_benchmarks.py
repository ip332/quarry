...
def generated_source_bytes(root: Path, backend: str) -> int:
    paths = {
    ...
    return sum(path.stat().st_size for path in paths[backend] if path.exists())
            ...
            for operation in operations:
                ...
                for backend in selected:
                    ...
                    value.update({
                        ...
                        "generated_source_bytes": generated_source_bytes(generated, backend),
                        "benchmark_binary_bytes": executable[backend].stat().st_size if backend != "python" else 0,
