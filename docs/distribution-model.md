...
| Generated Python code | Python backend output under caller-selected paths (PR-118 architecture; PR-119 scalar field support; PR-120 enum field support; PR-121 string/bytes field support) | Owned by the downstream project that generated it | Downstream-owned build artifact |
| Python runtime package | `runtime/python/` (`quarry-runtime-python` on PyPI, importable as `quarry.runtime.python`; PR-118 epoch constant, PR-119 `binary_record` scalar codec module, PR-120 `pack_enum`/`unpack_enum` additions, PR-121 `pack_string`/`unpack_string`/`pack_bytes`/`unpack_bytes` additions) | `pip install`-based, not CMake-installed or exported -- deliberately outside this document's CMake-centric distribution boundary, since Python has its own packaging ecosystem (see "Python Runtime Packaging" below) | Supported public SDK (distributed via pip, not CMake) |
